...
`NEXT_PUBLIC_APP_URL` вшивается в клиентский бандл на этапе сборки, поэтому
после смены домена сервиса нужен **новый build**, а не «Redeploy» — тот
переиспользует готовый образ, и в бандле остаётся старый адрес. Railway
освобождает прежний домен сразу, так что до пересборки браузер будет ходить
на мёртвый адрес и авторизация работать не станет.

Сам `${{RAILWAY_PUBLIC_DOMAIN}}` в переменных подставляется автоматически —
править `NEXT_PUBLIC_APP_URL` и `BETTER_AUTH_URL` руками не нужно.

Пересборку запускает любой коммит в подключённую ветку.
